...
test("versioned manifests identify Necktie 0.5.0", () => {
  for (const relative of jsonFiles) {
    const value = JSON.parse(fs.readFileSync(path.join(root, relative), "utf8"));
    assert.equal(value.version, "0.5.0", relative);
  ...
  assert.match(fs.readFileSync(path.join(root, "plugin.yaml"), "utf8"), /^version: 0\.5\.0$/m);
  assert.match(fs.readFileSync(path.join(root, "NOTICE"), "utf8"), /^Necktie 0\.5\.0$/m);
});

...
2. Extract the stable goal, audience, authorized discovery boundary, accepted source hierarchy, constraints, approved refinements, required deliverables, acceptance criteria, and verification requirements.
...
6. Convert every observable reference requirement into the deliverable contract: files, formats, sections, tables, columns, minimum coverage, citation rules, and target-environment checks.
7. Require fresh execution from raw accepted sources. Label prior outputs as hypotheses, structure, or style references unless independently supported.
8. Include a reviewer contract with an exact decision vocabulary and stopping rule.
9. Make the brief specific enough to execute yet independent of a particular model, tool name, or unavailable session state.
...
- Which paths or attachments are authorized, and which candidates were deliberately excluded?
- What is in and out of scope?
- What must be delivered and in what form?
- Which reference-artifact requirements must be matched, and how will they be tested?
...
2. `Compilation notes` listing only material assumptions, excluded candidates or prior-output claims, and unresolved authority or evidence limitations.
